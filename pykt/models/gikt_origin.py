...
class GIKT(Module):
    ...
    def __init__(self, num_question, num_skill, q_neighbors, s_neighbors, qs_table, emb_type="qid",agg_hops=3, emb_dim=100,
                 dropout=(0.2, 0.2), hard_recap=True, pre_train=False):
        ...
        # self.gru2 = GRUCell(emb_dim, emb_dim) (200,100)
    ...
    def forward(self, question, response, mask):
        question, response, mask = question.to(device), response.to(device), mask.to(device)
        ...
        h1_pre = torch.nn.init.xavier_uniform_(torch.zeros(self.emb_dim).to(device).repeat(batch_size, 1))
        h2_pre = torch.nn.init.xavier_uniform_(torch.zeros(self.emb_dim).to(device).repeat(batch_size, 1))
        state_history = torch.zeros(batch_size, seq_len, self.emb_dim).to(device)
        ...
        for t in range(seq_len - 1):  # 第t时刻
            ...
            emb_question_t = torch.zeros(batch_size, self.emb_dim).to(device)
            ...
            for i in range(batch_size):
                ...
                if len(skills_index.shape) == 0:  # 只有一个技能
                    skills_related_list.append(
                        torch.unsqueeze(self.emb_table_skill(skills_index), dim=0))  # [1, emb_dim]
                else:  # 不止一个技能
